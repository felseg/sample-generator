...
class SampleDataset(torch.utils.data.Dataset):
    def __init__(self, paths, global_args):
        ...
        self.augs = torch.nn.Sequential(
            PadCrop(global_args.sample_size,
                    randomize=global_args.random_crop),
            RandomPhaseInvert(),
        )
    ...
    def get_data_range(self):  # for parallel runs, only grab part of the data
        ...
        try:
            ...
            print("local_rank, world_size, start, stop =",
    ...
    def preload_files(self):
        ...
        # //8 to avoid FS bottleneck and/or too many processes (b/c * num_gpus)
        with Pool(processes=cpu_count()) as p:
            self.audio_files = list(
                tqdm.tqdm(p.imap(wrapper, range(start, stop)), total=stop-start))
